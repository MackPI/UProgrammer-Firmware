...
void set_sigma_delta_duty(unsigned char duty){
	...
	if (GPIO_REG_READ(GPIO_SIGMA_DELTA) & 0x10000){ // check to see if ΣΔ is running
		GPIO_REG_WRITE(GPIO_SIGMA_DELTA, 0);// turn off ΣΔ
		GPIO_REG_WRITE(GPIO_SIGMA_DELTA, 0x10000 | (prescaler<<8) | duty); // set Target Prescaler and restore duty
	} else {
		GPIO_REG_WRITE(GPIO_SIGMA_DELTA,  (prescaler<<8)|duty); // set Target Prescaler and restore duty but leave turned off
	}
